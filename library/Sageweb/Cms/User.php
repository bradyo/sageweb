...
/**
 * Doctrine ORM Base class for User model.
 *
 * @property integer $id
 ...
 * 
 * @property Sageweb_Cms_Profile $profile
 *
 * @author Brady Olsen <bradyo@uw.edu>
 */
class Sageweb_Cms_User extends Doctrine_Record
{
    /**
    ...
    {
        parent::setUp();
        $this->hasOne('Sageweb_Cms_Profile as profile', array(
            'local' => 'id',
            'foreign' => 'user_id',
            'owningSide' => false,
        ));
    ...
    public function isModerator()
    {
        ...
        $isModerator = ($this->role == Sageweb_Cms_User::ROLE_MODERATOR);
        $isAdmin = ($this->role == Sageweb_Cms_User::ROLE_ADMIN);
        return ($isModerator || $isAdmin);
    }

    public function isAdmin()
    {
        return ($this->role == Sageweb_Cms_User::ROLE_ADMIN);
    }
    ...
    public function canEdit(Sageweb_Cms_Abstract_Post $post)
    {
        switch ($post->entity->type) {
            ...
            case Sageweb_Cms_Entity::TYPE_JOB:
            case Sageweb_Cms_Entity::TYPE_COMMENT:
            case Sageweb_Cms_Entity::TYPE_DISCUSSION:
//            case Sageweb_Cms_Entity::TYPE_QUESTION:
//            case Sageweb_Cms_Entity::TYPE_ANSWER:
    ...
    public function vote($entity, $value)
    {
        ...
        if (!$existingVote) {
            // creating a new vote
            $vote = new Sageweb_Cms_EntityVote();
        ...
        Sageweb_Cms_Table_Entity::recomputeRating($entity->id);
    ...
    {
        $revision = new Sageweb_Cms_EntityRevision();
        $revision->entityId = $entity->id;
        $revision->status = Sageweb_Cms_EntityRevision::STATUS_PENDING;
    ...
    public function acceptRevision($revision, $comment = null)
    {
        ...
        $revision->status = Sageweb_Cms_EntityRevision::STATUS_ACCEPTED;
    ...
    public function rejectRevision($revision, $comment = null)
    {
        ...
        $revision->status = Sageweb_Cms_EntityRevision::STATUS_REJECTED;
        $revision->reviewerId = $this->id;
        $revision->reviewedAt = date('Y-m-d H:i:s');
        $revision->reviewerComment = $comment;
        $revision->save();
    ...
    public function updateRevision($revision, $comment = null)
    {
        ...
        $revision->reviewerComment = $comment;
        $revision->save();
        if ($revision->status == Sageweb_Cms_EntityRevision::STATUS_ACCEPTED) {
            $revision->apply();
        }
        return $revision;
    }

...
class DiscussionController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        $forum = Sageweb_Cms_Table_Forum::findOneBySlug($forumSlug);
        if ($forum) {
        ...
        $pager = Sageweb_Cms_Table_Discussion::getSearchPager($queryString, $sort);
        $pager->setItemCountPerPage(self::ITEMS_PER_PAGE);
        $pager->setCurrentPageNumber($this->_getParam('page', 1));

        // fetch tags in this forum
        $tags = Sageweb_Cms_Table_Discussion::getForumTags($forumId);
    ...
    public function rssAction()
    {
        ...
        $forum = Sageweb_Cms_Table_Forum::findOneBySlug($slug);
        if ($forum) {
            $forumId = $forum->id;
        }

        $tag = $this->_getParam('tag');
        $posts = Sageweb_Cms_Table_Discussion::findNewest($forumId, $tag);
    ...
    public function showAction()
    {
        ...
        $post = Sageweb_Cms_Table_Discussion::findOneById($id);
        if (!$post) {
        ...
        $tags = Sageweb_Cms_Table_Discussion::getTags($post->id);
    ...
    public function newAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            if ($form->isValid($this->getRequest()->getParams())) {
                ...
                $type = Sageweb_Cms_Entity::TYPE_DISCUSSION;
    ...
    public function editAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            if ($form->isValid($this->_getAllParams())) {
                ...
                if ($viewingUser->isModerator()) {
                    $reviewerComment = $formValues['reviewerComment'];
                    $revison->status = Sageweb_Cms_EntityRevision::STATUS_ACCEPTED;
